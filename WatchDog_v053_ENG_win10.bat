...
cls  
echo.
ECHO             Detected [92m%mygpu%[0m GPUs.
echo.
...
ECHO      Average loading of %mygpu% GPUs: [92m%gpu_average%%%[0m
...
echo.
ECHO [96m      Power consumption GPUs: %power_total% W [0m
ECHO [96m     Power consumption total: %power_kpd% W [0m
...
:rigerror
echo.
ECHO      Average loading of %mygpu% GPUs: [93m%gpu_average%%%[0m

echo.
ECHO            Our ping [92m%ping_time%[0m ms - OK.
...
FOR /L %%A IN (%RebootTimer%,-1,0) DO (
    cls
    echo.
    ECHO [41m    Unfortunately, miner not working.    [0m
    echo.
    ECHO       System reboot in [91m%%A[0m seconds.
    timeout /t 1 >nul  
)
shutdown.exe /r /t 00
