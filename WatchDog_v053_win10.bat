:: v05 2017-11-28 Добавлено сохранение скриншотов в Logs. Изменены тайминги опроса ошибки. Добавлены циклы ложного срабатывания. Прерывание на старте. Режим мониторинга без перезагрузки.
:: v04 2017-10-25 Добавлено телеграмм-оповещение
:: v03 2017-10-22 Изменена функция записи времени в логфайл
:: v02 2017-10-16 Добавлено логирование перезагрузок
:: Автор Egorka10 & co-authors
...
cls  
echo.
echo       Было обнаружено [92m%mygpu%[0m видеокарт.
echo.
echo   Ожидание загрузки программы майнера.
ECHO.
...
echo.
echo [92m      Майнинг криптовалют работает[0m
echo [97m        %DiffTimeDay% дней %DiffTimeHour% часов %DiffTimeMin% минут[0m
echo.
echo        Драйвер: %driver_version% Порог: %delta%%%
echo     Средняя загрузка %mygpu% видеокарт: [92m%gpu_average%%%[0m
...
echo.
echo [97m    GPU:   MHz:   Temp   Fan:   Power[0m
...
echo.
echo [96m     Потребление видеокарт: %power_total% Вт[0m
echo [96m    Потребление из розетки: %power_kpd% Вт [0m
...
if %ping_time% LSS 0 (
    cls
    echo.
    echo    Куда-то пропал интернет... Ждёмс...
    echo.
    echo               Пинг [95m%ping_time%[0m ms.
    timeout /t 15 >nul
    goto :starterror
)
...
FOR /L %%A IN (%IssueTimer%,-1,0) DO (
    cls
    echo.
    echo [91m                 ВНИМАНИЕ                [0m
    echo [91m      Сбой работы программы майнера      [0m
    echo.
    echo      Ожидание восстановления майнинга
    echo               Раунд %GPUIssueCount% из %GPUIssueCountMax%
    echo.           Осталось [92m%%A[0m секунд.   
    timeout /t 1 >nul
)
...
:rigerror
echo.
echo    Средняя загрузка %mygpu% видеокарт: [93m%gpu_average%%%[0m
echo.
echo            Наш пинг [92m%ping_time%[0m ms - OK.
echo     С интернетом проблем похоже НЕТ.
timeout /t 5 >nul
...
FOR /L %%A IN (%RebootTimer%,-1,0) DO (
    cls
    echo.
    echo [41m    Печально, но майнер не заработал.    [0m
    echo.
    echo   Перезагрузка системы через [91m%%A[0m секунд.
    timeout /t 1 >nul  
)
shutdown.exe /r /t 00
